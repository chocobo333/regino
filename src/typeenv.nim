...
proc Var*(_: typedesc[Value], env: TypeEnv): Value =
    ...
    env.tvs.incl result
proc Select*(_: typedesc[Value], types: seq[Value], env: TypeEnv): Value =
    result = Value(kind: ValueKind.Select, types: types.toHashSet, id: getTvId())
    env.selects.incl result
...
proc addImport*(self: TypeEnv, uri: string) = 
    self.scope.imports.add uri
